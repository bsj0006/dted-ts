...
class DataSetIdentifictionRecord {
  ...
  constructor(dsiStr: string) {
    ...
    this.clockwiseOrientationAngle =
      parseInt(dsiStr.slice(264, 267)) +
...
class AccuracyDescriptionRecord {
  ...
  constructor(accStr: string) {
    ...
    assert(
      this.multipleAccuracyOutlineFlag == 0 ||
        (this.multipleAccuracyOutlineFlag >= 2 && this.multipleAccuracyOutlineFlag <= 9)
    )
...
class DataRecord {
  ...
  constructor(record: Uint8Array) {
    assert(
      record[0] === 0o252,
      "Data record has incorrect starting value. Expected 170. Received " + record[0]
    )
    this.data = record
  }
  ...
  public validateChecksum(): boolean {
    ...
    let expectedChecksum =
      (this.data[i] << 24) | (this.data[i + 1] << 16) | (this.data[i + 2] << 8) | this.data[i + 3]
...
class DtedTile {
  ...
  constructor(fileBuffer: Buffer, options: Partial<DtedTileOptions> = {}) {
    ...
    assert(
      fileBuffer.length > DATA_RECORD_OFFSET,
      "Not enough bytes to read DTED data. Incorrect file length."
    )

    this.uhl = new UserHeaderLabel(fileBuffer.toString("ascii", UHL_OFFSET, UHL_LENGTH))
    this.dsi = new DataSetIdentifictionRecord(
      fileBuffer.toString("ascii", DSI_OFFSET, DSI_OFFSET + DSI_LENGTH)
    )
    this.acc = new AccuracyDescriptionRecord(
      fileBuffer.toString("ascii", ACC_OFFSET, ACC_OFFSET + ACC_LENGTH)
    )
    this.records = []
    ...
    for (var i = 0; i < this.dsi.longitudeLineCount; i++) {
      ...
      this.records.push(
        new DataRecord(fileBuffer.subarray(recordStart, recordStart + dataRecordLength))
      )
    }
    ...
    if (options_internal.validateChecksum) {
      this.records.forEach((record) =>
        assert(record.validateChecksum(), "Expected DTED data to pass checksum")
      )
    }
  }
  ...
  public getMaxElevation(): number {
    if (this.maxElevation === undefined) {
      this.maxElevation = this.records
        .map((record) => record.getMaxElelvation())
  ...
  public getMinElevation(): number {
    if (this.minElevation === undefined) {
      this.minElevation = this.records
        .map((record) => record.getMinElelvation())
        .reduce((prev, curr) => (prev < curr ? prev : curr))
    }
  ...
  public contains(latitude: number, longitude: number): boolean {
    return (
      latitude <= this.getNorthLatitude() &&
      latitude >= this.getSouthLatitude() &&
      longitude <= this.getEastLongitude() &&
      longitude >= this.getWestLongitude()
    )
  }

  /**
   * Gets the elevation (relative to EGM96 or MSL) at the latitude and longitude closest to the provided coordinate.
   * @param latitude Number from -90 and 90.
   * @param longitude Number from -180 to 180.
   */
...
function parseDDMMSSH(str: string, offset: number): number {
  ...
  let latitudeDeg: number =
    parseInt(str.slice(offset, offset + 2)) +
...
function parseDDDMMSSH(str: string, offset: number): number {
  let angleDeg: number =
    parseInt(str.slice(offset, offset + 3)) +
 ...
 */
async function readDtedAsync(
  filePath: string,
  options?: Partial<DtedTileOptions>
): Promise<DtedTile> {
  //Attempt to read in DTED file
...
export {
  UserHeaderLabel,
  DataSetIdentifictionRecord,
  AccuracyDescriptionRecord,
  AccuracySubregionDescription,
  CoordinatePairDescription,
  DataRecord,
  DtedTile,
  DtedTileOptions,
  isVoidElevation,
